...
public class SysUserController {
     ...
     * @return
     */
    @SysOperaLog(descrption = "获取本人信息")
    @GetMapping("/info")
    public R getUserInfo() {
        return R.ok(userService.findByUserInfoName(SecurityUtil.getUser().getUsername()));
    }
    @SysOperaLog(descrption = "获取班组内人员信息")
    @GetMapping("/infoList")
    public R getUserInfoByteam(Integer teamId) {
        return R.ok(userService.findByUserInfoTeamId(teamId));
    }
    /**
     * 修改密码
     *
