...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    public SysUser findByUserInfoName(String username) {
        SysUser sysUser = baseMapper.selectOne(Wrappers.<SysUser>lambdaQuery()
                .select(SysUser::getUserId, SysUser::getUsername, SysUser::getPhone, SysUser::getEmail, SysUser::getPassword, SysUser::getDeptId,SysUser::getTeamId, SysUser::getJobId, SysUser::getAvatar)
                .eq(SysUser::getUsername, username));

        List<SysUserRole> sysUserRoles= userRoleService.selectUserRoleListByUserId(sysUser.getUserId());
        Iterator<SysUserRole> iterator = sysUserRoles.iterator();
        // 根据相应的用户的id,查询相应发用户的角色列表，因为可能有多个角色，于是进行遍历list集合并将相应的值取出来拼接到相应的role之中；
        String ueserRole="";
        while (iterator.hasNext()) {
            SysUserRole good = iterator.next();
            System.out.println(good.getRoleId());
                      ueserRole=ueserRole+good.getRoleId().toString()+",";
            System.out.println(ueserRole);
            }
        // 获取部门
        sysUser.setRoleString(ueserRole);
...
//        sysUser.setJobName(jobService.selectJobNameByJobId(sysUser.getJobId()));
    ...
    // 获取班组内的人员信息
    @Override
    public List<SysUser> findByUserInfoTeamId(Integer teamId) {
       List<SysUser>  sysUser = baseMapper.selectList(Wrappers.<SysUser>lambdaQuery()
